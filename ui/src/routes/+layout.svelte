...
<div>
	...
	<div class="relative z-10 lg:hidden" role="dialog" aria-modal="true">
		...
		<div
			class="fixed inset-0 bg-gray-900/80 transition-opacity ease-linear duration-300 {sidebarOpen
				? 'opacity-100'
				: 'opacity-0 collapse'}"
			aria-hidden="true"
		></div>
		...
		<div
			class="fixed inset-0 flex transition ease-in-out duration-300 transform {sidebarOpen
				? 'translate-x-0'
				: '-translate-x-full'}"
		>
			...
			<div class="relative mr-16 flex w-full max-w-xs flex-1">
				...
				<div
					class="absolute left-full top-0 flex w-16 justify-center pt-5 ease-in-out duration-300 {sidebarOpen
						? 'opacity-100'
						: 'opacity-0'}"
				>
					<button
